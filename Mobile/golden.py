...
from Oracle.index import getMarket, getRandom
from Utils.functions import getClock, getDelay, getDuration, getHolidays, getSpan
from Utils.variables import STEP, HORIZON


# SAIDAS DO DESKTOP QUE O APP TEM DE REPRODUZIR (SORTEIO, FERIADOS, RELOGIO, ORACULO E MODELO), CONFERIDAS PELO TESTE EM KOTLIN
...
def getObs(rng, route, now, first, regime):
...
if __name__ == '__main__':
    ...
    for route in ROUTES:
        ...
        golden['market'].append({'route': route, 'ts': ts.tolist(), 'rain': rain.tolist(), 'price': price.tolist(), 'minutes': minutes.tolist()})
    ...
    if model.setup():
        ...
        frame = pd.DataFrame({'ts': rng.integers(getTs('2026-01-01'), getTs('2027-12-31'), 60), 'rain': np.append(getRain(rng, 56), [0.0, 12.0, 15.5, 0.25]), 'distance': rng.uniform(0.5, 300, 60), 'duration': rng.uniform(1, 240, 60), 'corridor': rng.uniform(0, 1, 60), 'tz': 'America/Sao_Paulo'})
        X = model.process(frame, *model.getClock(frame)[:2])
        golden['quantiles'] = {'frame': {key: frame[key].tolist() for key in ('ts', 'rain', 'distance', 'duration', 'corridor')}, 'raw': {column: {key: booster.predict(X).tolist() for key, booster in model.boosters[column].items()} for column in model.TARGETS}, 'sorted': {column: [q.tolist() for q in model.getQuantiles(model.boosters[column], X)] for column in model.TARGETS}}
        golden['model'] = []
        ...
        for route in ROUTES:
            for text in MOMENTS:
                ...
                for regime in ('nova', 'poucas', 'rastreada', 'choque'):
                    obs = getObs(rng, route, now, rain[0], regime)
                    out = model.get(route, grid, obs)
                    golden['model'].append({'route': route, 'regime': regime, 'ts': ts.tolist(), 'rain': rain.tolist(), 'obs': {key: obs[key].tolist() for key in obs}, 'out': {key: out[key].tolist() for key in out}})
        ...
        for route in ROUTES:
            ...
            for text in MOMENTS[:4]:
                ...
                price, minutes = getMarket(route, past, rain)
                database.set('INSERT OR IGNORE INTO Prices (route_id, ts, rain, price, minutes) VALUES (?, ?, ?, ?, ?)', zip([route['id']] * len(past), past.tolist(), rain.tolist(), price.tolist(), minutes.tolist()))
                engine.getWeather = lambda lat, lon: weather
                df = engine.get(row, now)
                del engine.getWeather
                golden['engine'].append({'route': route, 'now': now, 'weather': {key: weather[key].tolist() for key in weather}, 'today': {'ts': past.tolist(), 'rain': rain.tolist(), 'price': price.tolist(), 'minutes': minutes.tolist()}, 'out': {key: df[key].tolist() for key in df}})
