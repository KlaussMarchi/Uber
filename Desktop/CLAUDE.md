...
ride-hailing trip over the next 12 h in 10-min steps, for **Uber or 99**, chosen in a selector. Prices are
**simulated**: `Oracle/` holds one fare table per company and simulates the market; the model learns that oracle.
There is no free public pricing API for either company, so the only path to the real number is the user typing an
observed price into the *preço real no app* field, which refits that company's fare (`Fares` table). Addresses,
routes, weather and location come from real free services. `prompt.md` is the original specification; `README.md`
(Portuguese) documents the design choices and the measured results. `../Mobile` is the Android port (Kotlin +
Compose) that reads this app's `data/model.json` and reproduces the oracle, model and engine bit for bit.
...
python test.py          # full validation, ~15 min, needs network, opens a real Tk window, exit code 1 on failure
...
- Only `testUtils`, `testApi` and `testDistance` stand alone. `testBootstrap` points `database.path` and
  `model.path` to a temp folder and returns the routes the later tests receive; calling `testOracle`, `testTariff`,
  `testScenarios`, `testPrecision`, `testGeneralization`, `testShock`, `testRoutes`, `testWorker` or `testInterface`
  without it uses the real `data/`. `testWorker` needs `testRoutes` to have run: it only observes routes with
  `used_at > 0`, which are the ones a quote created.
- `testTariff` mutates the module-level `Oracle.FARES` and restores it at the end; a test added after it must not
  assume the published table without calling `oracle.update` with an empty frame first.
...
3. `Engine.get(route, company, now)` builds the grid: row 0 is the exact current instant, rows 1… are aligned
   ...
   covers the horizon. The "observed now" state comes from `Oracle.getState` and becomes money through
   `Oracle.getPrice`; today's earlier observations come from `Prices`, which stores the **market state**
   (`rain, excess, noise, minutes`), never a price — so switching company or recalibrating a fare never
   invalidates the history. `Model.get` returns `p10/p50/p90` and `m10/m50/m90`; `price`/`minutes`/`excess`/`noise`
   are filled only on row 0, and the worker writes those four back into `Prices`.
4. Failures reach the UI as `{'error': '<Portuguese message>'}`.

`Oracle` splits the market from the money. `getState(route, ts, rain)` returns the demand excess, the supply noise
and the travel minutes — all company-independent, because traffic and driver supply are the city's, not the app's.
`getPrice(distance, minutes, excess, noise, company)` turns that state into what one app charges, through `TARIFFS`
(base, km, minute, fee, floor, surge sensitivity, surge cap per company) and `getSurge`. `getMarket` is the two
composed. `update(rows)` refits `FARES` from the user's observed prices: a free level term (identifiable from one
observation) plus a ridge-regularised shape, each coefficient clamped to `LIMITS` and rounded to `DIGITS` decimals
**so the phone and the desktop land on exactly the same table despite different linear solvers**. The state is
deterministic by seed per local day; a local day whose midnight does not exist (DST) starts at the first existing
instant (`nonexistent='shift_forward'`).

`Model` predicts a **multiplier** over a base (`getTariff` without surge for price, free-flow duration for time),
because trees do not extrapolate distance. `company` is a categorical feature: `Model.update` expands every stored
market state into one training row per company (736 k states → 1.47 M rows, ~1.5 min per retrain), so a single
model learns Uber's stronger surge and 99's milder one. `FEATURES` is saved in `model.json` and `setup` rejects a
file whose feature list differs. Three layers: LightGBM quantile boosters per target × quantile; an
...
`Worker` is a daemon thread aligned to 10-min slots: on the first boot it bootstraps and trains (~2 min); then each
slot it observes the 5 most recently used routes (`used_at > 0`) into `Prices` (one state row per route) and stores
one forecast row per company in `Forecasts`. Consolidation is a **join**, not an update: `Worker.check` joins
`Forecasts` to `Prices` on (route, ts) and recomputes each company's observed price from the state, so `Forecasts`
carries no observed columns at all. It retrains every `RETRAIN` s. The UI only
...
whose company is no longer the selected one — otherwise a 99 series lands under the Uber baseline and fires a false
alert. Alert baselines and levels are keyed by `(route_id, company)`. Work goes
...
- Changing the `Prices`/`Routes` schema, the oracle or the bootstrap routes → bump `Database.VERSION`; the DB is
  dropped and the worker regenerates history and retrains. `Fares` is deliberately **not** dropped: those rows are
  the user's real observations, not synthetic data. Changing `Model.FEATURES` needs no manual step — `setup`
  compares the saved feature list and retrains on its own.
- `TARIFFS` is the published prior and `FARES` the table in use; never read `TARIFFS` where a price is computed.
  `Engine.setup()` must run after `database.setup()` (the worker does it) or the user's calibration is ignored.
- The Android app mirrors `Oracle` (including `getState`, `getPrice`, `getSurge` and the `update` fare fit),
  `Model.get`, `Model.getQuantiles`, `Engine.get`, `Locator.getStart` and the duration formatters line by line. After changing any of them, port the change to `../Mobile/app/src/main/java/com/klauss/tarifa`, run
  `venv/bin/python ../Mobile/golden.py data/model.json` and `./gradlew testDebugUnitTest` in `../Mobile`.
- Oracle calibration: on the reference route (`ORIGIN` → `DESTINATION` in `Utils/variables.py`) Uber must keep a
  median weekday 9–16 h dry price between R$ 54,67 and R$ 55,00, with rush + heavy rain peaks near R$ 90, and 99
  must land at 0,88–0,92 of it in that same window while staying strictly cheaper at every instant; `test.py`
  asserts all of it. Travel time must be identical across companies.
